...
public:
    ...
    void gen_led(COLOR_TYPE *color_val, byte val)
    {
        if (val > 0) {
            *color_val = randInt(0, MAX_LED_VAL);
        }
    }
    ...
    void on_init(void)
    {
        ...
        for (i = 0; i < HOR_LED_CNT; i++) {
            for (j = 0; j < VER_LED_CNT; j++) {
                ...
                gen_led(&(cur_cl->r), (inc_val[i][j] >> 4) & 0x3);
                gen_led(&(cur_cl->g), (inc_val[i][j] >> 2) & 0x3);
                gen_led(&(cur_cl->b), inc_val[i][j] & 0x3);
            }
        }
    }
